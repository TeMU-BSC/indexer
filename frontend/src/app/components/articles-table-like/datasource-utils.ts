...
import { Observable, concat, of, combineLatest, defer } from 'rxjs'
...
/** RxJs operator to sort an array based on an Observable of material Sort event */
export function sortRows<U>(
  ...
  return (rows$: Observable<U[]>) => combineLatest(
    rows$,
    sort$ ? sort$.pipe(toSortFn(sortFns, useDefault)) : null,
    (rows, sortFn) => {
